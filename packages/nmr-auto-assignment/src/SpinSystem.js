...
class SpinSystem {
    ...
    init() {
        ...
        try {
            ...
            if (this.cosy.length() > 0 && this.connCosy.length() > 0) {
                this.cosyT = new Array(nH);
                for(var cosyRow : this.cosyT)
                    cosyRow = new Array(nH);
                //To parse the theoretical COSY
                for (var pair: this.connCosy) {
                    let row = diaIDsH.indexOf(pair.fromDiaID);
                    let col = diaIDsH.indexOf(pair.toDiaID);
                    if (row >= 0 && col >= 0) {
                ...
                for(var cosyRow : this.cosyE)
                    cosyRow = new Array(nSignals);

                /*let resolutionX = Double.MAX_VALUE / 6;//((JSONObject)hmbc.get(0)).getDouble("resolutionX");
                if (((JSONObject)cosy.get(0)).has("resolutionX"))
                    resolutionX = ((JSONObject)cosy.get(0)).getDouble("resolutionX");

                for (let i = cosy.length() - 1; i >= 0; i--) {
                    double x = 0, y = 0;
