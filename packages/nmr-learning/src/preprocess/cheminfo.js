...
export function load(path, datasetName, options) {
  ...
  let molFiles = FS.readdirSync(path).filter((line) => {
  ...
  for (let i = 0; i < max; i++) {
    try {
      let molfile = loadFile(path + molFiles[i]);
      ...
      let jcampFile = molFiles[i]
        .replace('mol_', 'h1_')
        .replace('.mol', '.jdx');
      let spectraData1H = createSpectraData(path + jcampFile);
      ...
      for (let j = signals.length - 2; j >= 0; j--) {
        if (
          Math.abs(
            signals[j].to +
              signals[j].from -
