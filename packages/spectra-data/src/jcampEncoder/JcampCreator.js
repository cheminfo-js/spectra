...
export default class JcampCreator {
  ...
  convert(spectraData, options) {
    ...
    let scale = factorY / spectraData.getParamDouble('YFACTOR', 1);
    let minMax = {};
    ...
    let scaleX = Math.abs(1.0 / spectraData.getDeltaX());

    outString += `##TITLE= ${spectraData.getTitle()}${CRLF}`;
    outString += `##JCAMP-DX= 5.00\t$$${version}${CRLF}`;
    outString += `##OWNER= ${spectraData.getParamString(
      '##OWNER=',
      '',
    )}${CRLF}`;
...
function ntuplesHead(
  ...
  let outString = '';
  let variableX = spectraData.getSpectraVariable(0);
  let variableY = spectraData.getSpectraVariable(1);
  let variableZ = spectraData.getSpectraVariable(2);

  outString += `##DATA CLASS= NTUPLES${CRLF}`;
  outString += `##NUM DIM= 2${CRLF}`;
  let nTuplesName = spectraData.getDataType().trim();
  ...
  let isNMR = spectraData.getDataType().indexOf('NMR') >= 0;

  if (isNMR) {
    outString += `##.OBSERVE FREQUENCY= ${spectraData.getParamDouble(
      'observefrequency',
      0,
    )}${CRLF}`;
    outString += `##.OBSERVE NUCLEUS= ^${spectraData.getNucleus()}${CRLF}`;
    outString += `##$DECIM= ${spectraData.getParamDouble('$DECIM', 0)}${CRLF}`;
    outString += `##$DSPFVS= ${spectraData.getParamDouble(
      '$DSPFVS',
      0,
    )}${CRLF}`;
    outString += `##$FCOR= ${Math.floor(
      spectraData.getParamDouble('$FCOR', 0),
    )}${CRLF}`;
    ...
    outString += `##$TD= ${Math.floor(
      spectraData.getParamDouble('$TD', 0),
    )}${CRLF}`;
    outString += `##$BF1= ${spectraData.getParamDouble('$BF1', 0)}${CRLF}`;
    outString += `##$GRPDLY= ${spectraData.getParamDouble(
      '$GRPDLY',
      0,
    )}${CRLF}`;
    outString += `##.DIGITISER RES= ${spectraData.getParamInt(
      '.DIGITISER RES',
      0,
    )}${CRLF}`;
    outString += `##.PULSE SEQUENCE= ${spectraData.getParamString(
      '.PULSE SEQUENCE',
      '',
    )}${CRLF}`;
    ...
    if (spectraData.containsParam('2D_X_FREQUENCY')) {
      outString += `##$SFO1= ${spectraData.getParamDouble(
        '2D_X_FREQUENCY',
        0,
      )}${CRLF}`;
    } else {
    ...
    if (spectraData.containsParam('2D_X_OFFSET')) {
      outString += `##$OFFSET= ${spectraData.getParamDouble(
        '2D_X_OFFSET',
        0,
      )}${CRLF}`;
    }
    ...
    if (spectraData.is2D()) {
      ...
      if (spectraData.containsParam('2D_Y_FREQUENCY')) {
        ...
        outString += `##$SFO2= ${spectraData.getParamDouble(
          '2D_Y_FREQUENCY',
          0,
        )}${CRLF}`;
        outString += `##$BF2= ${spectraData.getParamDouble(
          '2D_Y_FREQUENCY',
          0,
        )}${CRLF}`;
      }
  ...
  if (isComplex) {
    outString += `##VAR_NAME=\t${spectraData.getXUnits()},\t${nTuplesName.substring(
      4,
    )}/REAL,\t${nTuplesName.substring(4)}/IMAG${CRLF}`;
    ...
    outString += `##UNITS=\tHZ,\t${spectraData.getYUnits()},\t${
      variableZ.units
    }${CRLF}`;
    outString += `##FACTOR=\t${1.0 / scaleX},\t${1.0 / scale},\t${1.0 /
      scale}${CRLF}`;
    ...
    outString += `##FIRST=\t${spectraData.getFirstX() *
      freq1},\t${spectraData.getY(0)},\t0${CRLF}`;
    outString += `##LAST=\t${spectraData.getLastX() *
      freq1},\t${spectraData.getLastY()},\t0${CRLF}`;
  } else {
    freq1 = 1;
    if (spectraData.is2D()) {
      outString += `##VAR_NAME=\tFREQUENCY1,\tFREQUENCY2,\tSPECTRUM${CRLF}`;
      outString += `##SYMBOL=\tF1,\tF2,\tY${CRLF}`;
      outString += `##.NUCLEUS=\t${spectraData.getNucleus(
        2,
      )},\t${spectraData.getNucleus(1)}${CRLF}`;
      ...
      outString += `##FIRST=\t${spectraData.getParamDouble('firstY', 0) *
        freq1},\t${spectraData.getFirstX() * freq2},\t${spectraData.getY(
        0,
      )}${CRLF}`;
      outString += `##LAST=\t${spectraData.getParamDouble('lastY', 0) *
        freq1},\t${spectraData.getLastX() * freq2},\t${spectraData.getY(
        spectraData.getNbPoints() - 1,
      )}${CRLF}`;
    } else {
      ...
      outString += `##FIRST=\t${variableX.first * freq1},\t${
        variableY.first
      },\t${variableZ.first}${CRLF}`;
      outString += `##LAST=\t${variableX.last * freq1},\t${variableY.last},\t${
        variableZ.last
      }${CRLF}`;
    }
  }
  ...
  if (userDefinedParams !== null) {
    for (let i = userDefinedParams.length - 1; i >= 0; i--) {
      if (spectraData.containsParam(userDefinedParams[i])) {
        outString += `##${userDefinedParams[i]}= ${spectraData.getParam(
          userDefinedParams[i],
          '',
        )}${CRLF}`;
      }
  ...
  for (sub = 0; sub < spectraData.getNbSubSpectra(); sub++) {
    ...
    if (spectraData.isDataClassPeak()) {
      ...
      for (let point = 0; point < spectraData.getNbPoints(); point++) {
        outString += `${spectraData.getX(point)}, ${spectraData.getY(
          point,
        )}${CRLF}`;
      }
      ...
      let tempString = '';
      let data = new Array(spectraData.getNbPoints());
      ...
      tempString += encode(
        data,
        spectraData.getFirstX() * scaleX,
        spectraData.getDeltaX() * scaleX,
        encodeFormat,
      );
...
function simpleHead(
  ...
  if (spectraData.getDataType().indexOf('NMR') >= 0) {
    ...
    outString += `##.OBSERVE NUCLEUS= ^${spectraData.getNucleus()}${CRLF}`;
    outString += `##$DECIM= ${Math.round(
      spectraData.getParamDouble('$DECIM', 0),
    )}${CRLF}`;
    outString += `##$DSPFVS= ${Math.round(
      spectraData.getParamDouble('$DSPFVS', 0),
    )}${CRLF}`;
    outString += `##$FCOR= ${Math.round(
      spectraData.getParamDouble('$FCOR', 0),
    )}${CRLF}`;
    outString += `##$SW_h= ${spectraData.getParamDouble('$SW_h', 0)}${CRLF}`;
    outString += `##$SW= ${spectraData.getParamDouble('$SW', 0)}${CRLF}`;
    outString += `##$TD= ${Math.round(
      spectraData.getParamDouble('$TD', 0),
    )}${CRLF}`;
    outString += `##$GRPDLY= ${spectraData.getParamDouble(
      '$GRPDLY',
