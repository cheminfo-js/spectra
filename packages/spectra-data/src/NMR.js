...
export default class NMR extends SD {
  ...
  static fromSignals(prediction, options = {}) {
    ...
    let data = simulate1D(spinSystem, options);
  ...
  static fromXY(x, y, options) {
    ...
    let spectraData = SD.fromXY(x, y, options);
    let spectrum = spectraData.sd.spectra[0];
    ...
    spectraData.putParam(
      '$SW_h',
      Math.abs(spectrum.lastX - spectrum.firstX) * spectrum.observeFrequency,
    );
   ...
   */
  getMagnitude() {
    return new NMR(Filters.absoluteValue(this));
  }
  ...
  getRanges(options = {}) {
    if (this.ranges) {
      ...
      let ranges = peaks2Ranges(this, peaks, options);
      return ranges;
    }
  }
  ...
  fillImpurity(solvent, options = {}) {
    const { impurity = null, value = 0, error = 0.025 } = options;
